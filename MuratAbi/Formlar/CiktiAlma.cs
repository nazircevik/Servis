...
namespace MuratAbi.Formlar
{
    public partial class CiktiAlma : Form
    {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            //logo için
            e.Graphics.DrawImage(Properties.Resources.TVS_GEÇİT_MOTOR, 100, 20);

            //Bu kısımda sipariş formu yazısını ve çizgileri yazdırıyorum
            StringFormat myStringFormat = new StringFormat();
            myStringFormat.Alignment = StringAlignment.Far;
            myFont2 = new Font("Calibri", 15, FontStyle.Bold);
            ...
            e.Graphics.DrawString("Plaka   : " , myFont2, sbrush, 100, 260);
            e.Graphics.DrawString(motor.Plaka, myFont2, sbrush, 170, 260);

            e.Graphics.DrawString("Marka : " ,myFont2, sbrush, 100, 290);
            e.Graphics.DrawString(motor.Marka, myFont2, sbrush, 170, 290);

            e.Graphics.DrawString("Model : " , myFont2, sbrush, 100, 320);
            e.Graphics.DrawString( motor.Model, myFont2, sbrush, 170, 320);

            e.Graphics.DrawString("Renk    : " + motor.Renk, myFont2, sbrush, 99, 350);

            e.Graphics.DrawString("Ad                    : " + cari.Ad, myFont2, sbrush, 450, 260);
            e.Graphics.DrawString("Soyad              : " + cari.Soyad, myFont2, sbrush, 450, 290);
            e.Graphics.DrawString("CepTelefonu  : " + cari.CepTelefonu, myFont2, sbrush,450, 320);
            e.Graphics.DrawString("Geliş Km         : " + servis.GelKm, myFont2, sbrush, 450, 350);
            e.Graphics.DrawString("Geliş Tarihi     : " + servis.Tarih, myFont2, sbrush, 450, 380);

            e.Graphics.DrawString("Adres    : " + cari.Adres, myFont2, sbrush, 100, 490);
            e.Graphics.DrawString("Şikayet  : " + servis.Not, myFont2, sbrush, 100, 520);



            e.Graphics.DrawLine(myPen, 120, 720, 750, 720);

            myFont = new Font("Calibri", 12, FontStyle.Bold);
          
            e.Graphics.DrawString("PARÇA AÇIKLAMA ", myFont, sbrush, 240, 728);
      
            e.Graphics.DrawString("Fiyat", myFont, sbrush, 640, 728);

            e.Graphics.DrawLine(myPen, 120, 753, 750, 753);
            ...
            {
                e.Graphics.DrawString(item.ParcaAciklama, myFont, sbrush, 220, y);
                decimal bFiyat = Convert.ToDecimal(item.Tutar);
            
                gTotal += bFiyat;
                e.Graphics.DrawString(bFiyat.ToString("c"), myFont, sbrush, 700, y, myStringFormat);
